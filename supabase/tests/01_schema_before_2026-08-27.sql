-- The `profiles`/`friends`/`results` schema as it stood BEFORE the 2026-08-27
-- security migration — i.e. what a database deployed from the old schema.sql is
-- actually running right now.
--
-- Committed as a snapshot rather than read out of git history on purpose: the RLS
-- gate needs a fixed "before" to open the holes against, and `HEAD:supabase/schema.sql`
-- stops being the before the moment the fix is committed. Do not edit this file to
-- track schema.sql — it is a fossil, and its whole value is that it does not move.
--
-- Source: supabase/schema.sql @ f672692.

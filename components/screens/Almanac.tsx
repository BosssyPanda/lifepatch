        ...
        <motion.div className="fixed inset-0 z-50 flex flex-col bg-bg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <div className="sticky top-0 z-10 border-b border-ink/12 bg-bg">
            <div className="mx-auto flex max-w-3xl items-center justify-between px-5 py-3">
              ...
              <button type="button" onClick={onClose} aria-label="Close almanac" className="rounded-[3px] border border-ink/20 p-2 text-ink-dim transition-colors hover:border-ink hover:text-ink">
                <CloseIcon size={18} />
              </button>
            </div>
                    ...
                    <Reveal key={i} delay={i * 0.03}>
                      <article className="rounded-[5px] border border-ink/12 bg-bg2 p-5">
                        <div className="flex items-start justify-between gap-3">
                          <p className="font-body text-[1.05rem] leading-snug text-ink">&ldquo;{m.claim}&rdquo;</p>
                        ...
                        <p className="mt-2 font-body text-sm leading-relaxed text-ink/80">{m.explain}</p>
                      </article>
                    </Reveal>
                  ))}
                </Section>
                    ...
                    {TERMS.map((t, i) => (
                      <Reveal key={t.term} delay={i * 0.02}>
                        <div className="rounded-[4px] border border-ink/12 bg-bg2 p-4">
                          <p className="font-mono text-sm font-semibold uppercase tracking-wide text-ink">{t.term}</p>
  ...
  return (
    <div>
      ...
      <ul className="mt-1 space-y-1">
        {items.map((it, i) => (
          <li key={i} className="flex gap-1.5 font-body text-sm leading-snug text-ink/85">

...
export function MuteButton({ muted, onToggle }: { muted: boolean; onToggle: () => void }) {
  return (
    <button
      ...
      className="pointer-events-auto flex items-center gap-1.5 rounded-[3px] border border-ink/25 bg-bg px-2.5 py-1.5 text-ink-dim transition-colors hover:border-ink hover:text-ink"
    >
      <Speaker muted={muted} />
...
export function SkipButton({ onSkip }: { onSkip: () => void }) {
  return (
    <button
      type="button"
      onClick={onSkip}
      className="pointer-events-auto flex items-center gap-1.5 rounded-[3px] border border-ink/25 bg-bg px-3 py-1.5 text-ink-dim transition-colors hover:border-ink hover:text-ink"
    >
